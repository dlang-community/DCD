<img src="dcd_lsp.png" alt="DCD" width="200"/>
...
the feature set focused on what DCD does well.
...
> document symbols, inlay hints, automatic import-path detection). Not yet
> implemented: incremental document sync, `workspace/symbol`, formatting,
> and `completionItem/resolve`. The semantic engine is shared with the classic
...
needing its own plugin for DCD's custom socket protocol. Being lean also
removes a layer: the editor talks straight to the semantic engine over
stdio, with no client process or socket hop in between, which lowers
latency.
...
A fully working extension lives in [`editors/code/`](editors/code). It is a
thin TypeScript client around `vscode-languageclient`; the interesting
logic is all in the server, so the extension stays small and easy to audit.
...
* Working:
	* Completion of built-in type properties, enums, and class/struct/interface instances
	* Call tips for functions, constructors, and function-type variables, with doc comments
	* *import* statement completions, including public imports
	* `alias` declarations, *alias this*, and *with* statements
	* `__traits`, `scope`, and `extern` arguments
	* Finding the declaration location of the symbol at the cursor
	* UFCS suggestions: concrete/fundamental/string/literal/pointer/array/alias receivers, bare template parameters (`void f(T)(T t)`), *alias this*, inheritance (derived receivers match base-class parameters), template instantiation identity (`Box!"a"` matches `Box!"a"` but not `Box!"b"`), slices, chaining, and call tips
	* String mixins generated by simple eponymous templates (partial support, see below)
	* Type inference: *auto* declarations from initializers, *foreach* loop variables (including function call results)
	* Context-aware completion: struct initializer field names, named arguments (`foo(al` offers parameter names), parameter names at bare argument positions (LSP), switch case labels, statement/declaration keywords (`as` offers `assert`)
* Not working:
	...
	* Autocompletion of declarations with template arguments (no `T` → `int` substitution)
...
## String mixins (partial support, new)

DCD does not run a CTFE engine, but it evaluates the common pattern of
string mixins generated by simple eponymous templates — when the argument
can be computed from string literals, `~` concatenation, and template
value parameters, the generated declarations become real symbols
(completion, go-to-definition, hover all work):

```d
enum GenStruct(string name) = "struct " ~ name ~ ";";
mixin(GenStruct!q{MyType});   // MyType becomes a real symbol
```

Also supported: all string literal forms, alias forwarders
(`alias Fwd(string name) = GenStruct!name;`), and mixins nested in the
generated code (recursion-capped). Not supported: arbitrary compile-time
function calls, `format`, `__traits` compositions, or anything needing
real CTFE.

Go-to-definition on a generated symbol points at the `mixin(...)` site.
...
1. Configure your text editor:
	* **If your editor supports LSP** (VS Code, Neovim, Emacs, Kate, Helix, ...), point it at `dcd-server --lsp`. See the [installation guide](editors/code/INSTALL.md) for editor-specific instructions, and [editors/code/](editors/code) for the VS Code extension.
