...
* Working:
	...
	* UFCS suggestions that respect template instantiation identity: a `Box!"a"` receiver matches `void f(Box!"a".Handle h)` but not `void f(Box!"b".Handle h)`, including through aliases (`alias BoxA = Box!"a";`) and call results (`getBox!"a"().foo()`)
	* UFCS suggestions for slices (`arr[1..2].` completes as the element type)
	* Dot chaining with other UFCS functions
	* UFCS call tips
	* String mixins generated by simple eponymous templates (partial support, see below)
	* *auto* declarations infer the type from the initializer (`auto a = Plain.init;`, `auto s = S(1);`)
...
string mixins generated by simple eponymous templates. When a mixin's
argument can be computed from string literals, `~` concatenation, and
template value parameters, the generated declarations are parsed and
produce real symbols (completion, go-to-definition, hover all work).

This is a new feature: previously such mixins produced no symbols at all.

Supported:

```d
// eponymous template with a single string value parameter
enum GenStruct( string name ) = "struct " ~ name ~ "; alias " ~ name ~ " = " ~ name ~ "_impl;";

mixin( GenStruct!q{MyType} );   // MyType and MyType_impl become real symbols
mixin( GenStruct!"MyType" );    // regular string literals work too
```

* string literals of all forms (`"..."`, `q{...}`, `r"..."`)
* `~` concatenation of literals and parameters
* alias forwarders: `alias Fwd(string name) = GenStruct!name;` followed by
  `mixin( Fwd!q{X} );`: a forwarding template that just passes its argument
  through to another mixin template works the same as calling it directly
* mixins nested inside the generated code (recursion-capped)

Not supported:

* arbitrary compile-time function calls, `format`, `__traits` compositions
* mixins whose argument depends on runtime values or complex CTFE
* `alias X(string name) = "..." ~ name;` (libdparse cannot parse an alias
  with template parameters whose body is an expression; the `enum` form
  works)

The generated symbols carry the file/offset of the `mixin(...)` declaration
itself, so go-to-definition on a generated symbol points at the mixin site.
