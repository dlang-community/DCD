...
if grep "DCD BUILD FAILED" <<< "$TOTAL"; then
	...
	echo '⚠️ The basic builds and tests passed, but the `profile-gc` statistics build failed (often a ldc2 `-profile=gc` issue, not a problem with this PR). GC statistics are unavailable for this run.'
	echo
else
