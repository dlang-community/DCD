 ...
 *
 */
public AutocompleteResponse getOutline(const AutocompleteRequest request)
{
	class Outliner : ASTVisitor
	{
		alias CompletionKind CK;
		static struct OutlineSymbol
		{
			string outScope;
			string fullname;
			CK kind;
		...
		OutlineSymbol[] getSymbols()
		{
			sort!((a, b) => a.outScope < b.outScope)(symbols);
			return symbols;
		}
	...
	AutocompleteResponse response;
	foreach (const ref Outliner.OutlineSymbol s; outliner.getSymbols())
	{
		response.completionKinds ~= s.kind;
		response.completions ~= s.outScope ~ '\t' ~ s.fullname;
		response.locations ~= s.location;
	}
	Log.info("Outline with ", response.completions.length, " elements");
